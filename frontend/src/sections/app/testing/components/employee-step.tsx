...
import { useTestingEmployeesQuery } from '../api/use-testing-api';
import { TestingEmployeesGridSkeleton } from '../skeleton';
...
export default function EmployeeStep({ onSelect }: Props) {
  ...
  const employeesQuery = useTestingEmployeesQuery(queryParams);
    ...
    <Stack spacing={0}>
      ...
      >
        ...
        <TextField
          ...
          placeholder={tx('testing.employee.searchPlaceholder')}
          size="small"
          sx={{
            width: { xs: '100%', sm: 280 },
      ...
      <Box sx={{ pt: 1.5, pb: { xs: 3, md: 4 } }}>
        ...
        {isLoading && (
          <Box sx={{ px: { xs: 3, md: 5 }, py: 1 }}>
            <TestingEmployeesGridSkeleton items={5} />
          </Box>
        )}

        {!isLoading && notFound && (
          <EmptyContent filled title={tx('testing.employee.empty')} sx={{ py: 10 }} />
        )}
