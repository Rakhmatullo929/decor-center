...
export default function TestingPanel({ children }: PropsWithChildren) {
  const { tx } = useLocales();
  ...
  return (
    <Box sx={{ position: 'relative' }}>
      <Container maxWidth={settings.themeStretch ? false : 'lg'}>
        <CustomBreadcrumbs
          heading={tx('testing.title')}
          links={[{ name: tx('common.appName'), href: paths.home }, { name: tx('testing.title') }]}
          sx={{ mb: { xs: 4, md: 6 } }}
        />
